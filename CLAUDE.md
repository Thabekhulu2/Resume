...
## For every new request or feature:
1. Create a GitHub ticket first, before any spec, plan, or code changes. Do not
   proceed further until the ticket exists.
2. Read spec_template.md and generate a full spec for the ticket using that
   template. Do not generate a plan or write code yet.
3. Based on the approved spec, write a step-by-step implementation plan. Do not
   implement anything — wait for explicit approval (Gate 1).
4. Once approved, implement the plan, then write/run relevant unit and E2E tests.
5. Do not run git commit without explicit approval tied to a specific ticket
   (Gate 2). If you notice other uncommitted or unrelated work in the repo,
   flag it — do not commit it automatically, even if it seems related.
6. Open a PR referencing the ticket (e.g. "Closes #N") so it closes automatically
   on merge. Wait for reviewer approval before deploying.

## Rules that apply throughout:
- One stage = one action. Do not jump ahead without being told to.
